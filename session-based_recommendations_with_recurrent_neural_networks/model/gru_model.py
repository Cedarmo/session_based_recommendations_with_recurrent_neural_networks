...
class GRU4Rec(object):
    ...
    def __init__(self, n_items, rnn_size, learning_rate, init_as_normal,
        ...
        self.grad_cap = grad_cap  # 是否对梯度加L2范数, grad_cap 大于 0, 表示加入L2范数，否则不加, 说白了就是
    ...
    if FLAGS.is_distributed == 1:
        ...
        if FLAGS.job_name == "ps":
            ...
            with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index,
                                                          cluster=cluster)):
                gru = GRU4Rec(n_items=2000, rnn_size=100, learning_rate=0.0001,
                              init_as_normal=True, sigma=0.1, decay_steps=1e1, decay_rate=0.96,
                ...
                with tf.train.MonitoredTrainingSession(master=server.target,
                    ...
                    while not sess.should_stop():
                        ...
                        feed_dict = {gru.X: batch_x, gru.Y: batch_y, gru.seq_len: seqLen, gru.drop_out: 0.5}
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(0, 2):
                ...
                feed_dict = {gru.X: batch_x, gru.Y: batch_y, gru.seq_len: seqLen, gru.drop_out: 0.5}
                o, s, yt, c, yp, step, _, _ = sess.run(fetches, feed_dict)
                print("step=", step, "output=", o, "state=", s, "yhatTrain=", yt, "yhatPredict=", yp, "cost=", c)


if __name__ == "__main__":
    FLAGS = tf.app.flags.FLAGS
    tf.app.flags.DEFINE_integer("is_distributed", 0, "是否分布式训练")

...
class GRU4Rec:
    ...
    def top1(self, yhat):
        yhatT = tf.transpose(yhat)
        term1 = tf.reduce_mean(tf.nn.sigmoid(-tf.diag_part(yhat)+yhatT)+tf.nn.sigmoid(yhatT**2), axis=0)      # axis=0 表示对列取均值
        term2 = tf.nn.sigmoid(tf.diag_part(yhat)**2) / self.batch_size   # 没看懂，求教
        return tf.reduce_mean(term1 - term2)

    def build_model(self):

        self.X = tf.placeholder(tf.int32, [self.batch_size], name='input')      # batch
        self.Y = tf.placeholder(tf.int32, [self.batch_size], name='output')     # batch
        self.state = [tf.placeholder(tf.float32, [self.batch_size, self.rnn_size], name='rnn_state') for _ in range(self.layers)]   # layers * batch * rnn_size
        ...
        with tf.variable_scope('gru_layer'):
            ...
            embedding = tf.get_variable('embedding', [self.n_items, self.rnn_size], initializer=initializer)    # n_items * rnn_size
            softmax_W = tf.get_variable('softmax_w', [self.n_items, self.rnn_size], initializer=initializer)    # n_items * rnn_size
            softmax_b = tf.get_variable('softmax_b', [self.n_items], initializer=tf.constant_initializer(0.0))  # n_items
            ...
            inputs = tf.nn.embedding_lookup(embedding, self.X)   # batch * rnn_size
            # output: batch * rnn_size       state: batch
        ...
        if self.is_training:
            ...
            '''
            sampled_W = tf.nn.embedding_lookup(softmax_W, self.Y)      # batch * rnn_size
            sampled_b = tf.nn.embedding_lookup(softmax_b, self.Y)      # batch
            logits = tf.matmul(output, sampled_W, transpose_b=True) + sampled_b   # batch * batch
            self.yhat = self.final_activation(logits)    # batch * batch
